static char help[] = "Particle fields.\n\
We calcualte the error in a particle representation of a field.\n\n\n";

/*
https://math.stackexchange.com/questions/1805184/the-dirac-delta-does-not-belong-in-l2/1805715
...
static PetscErrorCode SetupParticles(DM dm, DM *partdm, AppCtx *user)
{
  DM             sdm;
  PetscReal     *coords, *viscosities;
  PetscInt      *materials;
  PetscInt       particlesPerCell = 1;
  PetscInt       dim, d, cStart, cEnd, c, bs;
  PetscErrorCode ierr;
  ...
  for (c = cStart; c < cEnd; ++c) {
    ...
    for (d = 0; d < dim; ++d) coords[i+d] = centroid[d];
  ...
  ierr = DMViewFromOptions(sdm, NULL, "-particle_dm_view");CHKERRQ(ierr);
  *partdm = sdm;
  PetscFunctionReturn(0);
}
...
{
  DM             dm, pdm; /* Problem specification and particle layout */
  ...
  ierr = SetupParticles(dm, &pdm, &user);CHKERRQ(ierr);
